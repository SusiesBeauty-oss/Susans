...
const FallingSparkles = () => {
  const sparkles = Array.from({ length: 40 }).map((_, i) => {
    ...
    return {
      ...
      color: randomColor, char: Math.random() > 0.5 ? '✦' : '✧' 
    };
  });
...
const EmpowermentContainer = ({ children }) => (
  <div style={{ margin: '0 auto', maxWidth: '1000px', padding: '60px 40px', textAlign: 'center', position: 'relative', transition: 'all 0.5s ease' }}>{children}</div>
);

const EmpowermentButton = ({ text, onClick }) => (
  <button style={{
    backgroundColor: '#F2D4D7', color: '#5C5454', border: '1px solid #E8C5C8', padding: '12px 50px', fontSize: '32px', fontFamily: "'Alex Brush', cursive", fontWeight: '400',
...
const QuizOptionButton = ({ text, onClick }) => (
  <button style={{
    display: 'block', width: '100%', maxWidth: '450px', margin: '12px auto', backgroundColor: 'rgba(255, 255, 255, 0.7)', color: '#5C5454', border: '1px solid #E8C5C8',
    padding: '16px 25px', fontSize: '1.2rem', fontFamily: "'Cormorant Garamond', serif", cursor: 'pointer', transition: 'all 0.3s ease', borderRadius: '15px', letterSpacing: '1px'
  }}
  onClick={onClick}
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(92, 84, 84, 0.5)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'fadeIn 0.3s ease', padding: '20px', boxSizing: 'border-box' }} onClick={onClose}>
      <div style={{ backgroundColor: '#FFF9F9', border: '1px solid #E8C5C8', borderRadius: '20px', padding: '30px', maxWidth: '500px', width: '100%', boxShadow: '0 10px 30px rgba(232, 197, 200, 0.4)', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
        ...
        {product.description && <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem', color: '#736A6A', lineHeight: '1.6', margin: '0 0 20px 0' }} dangerouslySetInnerHTML={{ __html: product.description }} />}
        ...
        {hasVariants && product.variants.length > 1 && (
          <div style={{ marginBottom: '20px' }}>
            ...
            <select value={selectedVariantIdx} onChange={(e) => setSelectedVariantIdx(Number(e.target.value))} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #E8C5C8', fontFamily: "'Cormorant Garamond', serif", color: '#5C5454', backgroundColor: '#FFF9F9', outline: 'none' }}>
              {product.variants.map((v, idx) => (
                <option key={v.sku || idx} value={idx}>{v.variantName && v.variantName !== v.sku ? v.variantName : `Option ${idx + 1}`}</option>
              ))}
            </select>
          </div>
        )}
        <button 
          onClick={() => { onAddToCart({ ...product, sku: activeVariant?.sku || product.sku || product._id, price: activeVariant?.price || product.price, selectedVariantName: activeVariant?.variantName, imageUrl: activeVariant?.imageUrl || product.imageUrl, displayTitle: displayTitle }); onClose(); }}
...
const ProductCard = ({ product, onAddToCart, onCardClick }) => {
  ...
  return (
    <div onClick={() => onCardClick && onCardClick(enrichedProduct)} style={{ backgroundColor: 'rgba(255, 255, 255, 0.85)', border: '1px solid #E8C5C8', borderRadius: '15px', padding: '15px', textAlign: 'center', transition: 'transform 0.3s ease, box-shadow 0.3s ease', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 4px 15px rgba(232, 197, 200, 0.15)', height: '100%', cursor: 'pointer' }} onMouseOver={(e) => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.boxShadow = '0 8px 25px rgba(232, 197, 200, 0.3)'; }} onMouseOut={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 15px rgba(232, 197, 200, 0.15)'; }}>
      <div>
        <div style={{ width: '100%', height: '180px', backgroundColor: '#FFF0F2', borderRadius: '10px', marginBottom: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
          {product.essentialSlotLabel && (
            <span style={{ position: 'absolute', top: '8px', left: '8px', backgroundColor: 'rgba(92, 84, 84, 0.85)', color: '#FFFFFF', padding: '4px 10px', borderRadius: '12px', fontSize: '0.75rem', fontFamily: "'Cormorant Garamond', serif", letterSpacing: '0.5px', zIndex: 2 }}>{product.essentialSlotLabel}</span>
          )}
          {formattedImageUrl ? (
        ...
        <h4 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.15rem', color: '#5C5454', margin: '0 0 8px 0', minHeight: '45px', lineHeight: '1.3' }}>{displayTitle}</h4>
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.4rem', color: '#B38B8F', fontWeight: 'bold', margin: '0 0 10px 0' }}>${parseFloat(activeVariant?.price || product.price || 0).toFixed(2)}</p>

        {hasVariants && product.variants.length > 1 && (
          <div onClick={(e) => e.stopPropagation()}>
            <select value={selectedVariantIdx} onChange={(e) => setSelectedVariantIdx(Number(e.target.value))} style={{ width: '100%', padding: '8px', marginBottom: '15px', borderRadius: '8px', border: '1px solid #E8C5C8', fontFamily: "'Cormorant Garamond', serif", color: '#5C5454', backgroundColor: '#FFF9F9', outline: 'none' }}>
              {product.variants.map((v, idx) => (
                <option key={v.sku || idx} value={idx}>{v.variantName && v.variantName !== v.sku ? v.variantName : `Option ${idx + 1}`}</option>
              ))}
            </select>
      ...
      <button 
        onClick={(e) => { e.stopPropagation(); onAddToCart({ ...product, sku: activeVariant?.sku || product.sku || product._id, price: activeVariant?.price || product.price, selectedVariantName: activeVariant?.variantName, imageUrl: activeVariant?.imageUrl || product.imageUrl, displayTitle: displayTitle }); }}
...
const ElegantInput = ({ type, placeholder, name, value, onChange }) => (
  <input type={type} name={name} placeholder={placeholder} value={value} onChange={onChange} style={{ width: '100%', maxWidth: '380px', padding: '16px 25px', margin: '10px auto', display: 'block', backgroundColor: 'rgba(255, 255, 255, 0.8)', border: '1px solid #E8C5C8', borderRadius: '30px', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.2rem', color: '#5C5454', outline: 'none', boxSizing: 'border-box', transition: 'all 0.3s ease' }} onFocus={(e) => { e.target.style.backgroundColor = '#FFFFFF'; e.target.style.boxShadow = '0 0 10px rgba(232, 197, 200, 0.4)'; }} onBlur={(e) => { e.target.style.backgroundColor = 'rgba(255, 255, 255, 0.8)'; e.target.style.boxShadow = 'none'; }} />
);
...
function App() {
  ...
  const handleAddToCart = (product) => {
    setCart(prevCart => {
      ...
      if (existingItem) return prevCart.map(item => item.sku === product.sku ? { ...item, quantity: item.quantity + 1 } : item);
  ...
  const getCuratedRecommendations = () => {
    ...
    essentialSlots.forEach(slot => {
      ...
      if (candidates.length === 0) candidates = scoredProducts.filter(p => !usedProductIds.has(p._id || p.title));
    ...
    <div style={{ width: '100%', minHeight: '100vh', display: 'flex', alignItems: 'center', position: 'relative' }}>
      ...
      {(step === 1 || step === 5) && (
        <div style={{ position: 'fixed', top: '30px', right: '30px', zIndex: 1000 }}>
          <button onClick={() => setIsCartOpen(!isCartOpen)} style={{ backgroundColor: '#F2D4D7', border: '1px solid #B38B8F', borderRadius: '30px', padding: '10px 22px', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem', color: '#5C5454', cursor: 'pointer', boxShadow: '0 4px 15px rgba(232, 197, 200, 0.4)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span>🛍️ Ritual Bag</span>
            <span style={{ backgroundColor: '#B38B8F', color: '#FFFFFF', borderRadius: '50%', padding: '2px 8px', fontSize: '0.9rem', fontWeight: 'bold' }}>{cart.reduce((sum, item) => sum + item.quantity, 0)}</span>
          </button>
        </div>
      )}
      ...
      <EmpowermentContainer>
        {step === 0 && (
          <div style={{ animation: 'fadeIn 1s ease' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '40px auto' }}><div style={{ width: '80px', height: '1px', backgroundColor: '#E8C5C8' }}></div><span style={{ color: '#E8C5C8', margin: '0 15px', fontSize: '1.2rem' }}>✧</span><div style={{ width: '80px', height: '1px', backgroundColor: '#E8C5C8' }}></div></div>
            <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center' }}>
              ...
              <p style={{ cursor: 'pointer', color: '#A89999', textDecoration: 'underline', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem', transition: 'color 0.3s ease', marginTop: '10px' }} onClick={() => setStep(4)} onMouseOver={(e) => e.target.style.color = '#736A6A'} onMouseOut={(e) => e.target.style.color = '#A89999'}>Already a member? Sign in</p>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div style={{ animation: 'fadeIn 1s ease', textAlign: 'left' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
              ...
              <button onClick={() => setStep(0)} style={{ background: 'none', border: '1px solid #E8C5C8', borderRadius: '20px', padding: '8px 20px', color: '#736A6A', fontFamily: "'Cormorant Garamond', serif", cursor: 'pointer', fontSize: '1.1rem' }}>← Home</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
              {backendProducts.length > 0 ? backendProducts.map((product) => (
        ...
        {step === 2 && (
          <div style={{ animation: 'fadeIn 1.5s ease' }}>
            ...
            <p style={{ fontSize: '1.4rem', lineHeight: '1.8', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', margin: '15px auto 35px', maxWidth: '600px' }}>Unlock Susan's Consultation Quiz and bespoke curation.</p>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', flexWrap: 'wrap', marginBottom: '35px' }}>
              <div style={{ border: '1px solid rgba(232, 197, 200, 0.6)', borderRadius: '20px', padding: '40px 30px', backgroundColor: 'rgba(255, 255, 255, 0.7)', flex: '1', minWidth: '300px', maxWidth: '400px', boxShadow: '0 10px 30px rgba(232, 197, 200, 0.15)' }}>
                ...
                <ul style={{ textAlign: 'left', listStyle: 'none', padding: 0, margin: '0 0 35px 0', color: '#736A6A', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.25rem', lineHeight: '2' }}>
                  <li>✧ Full access to Curation Quiz</li><li>✧ 3-piece skin care ritual box</li><li>✧ Quarterly 1-on-1 consultation</li><li>✧ 10% boutique discount</li>
                </ul>
              ...
              <div style={{ border: '1px solid rgba(179, 139, 143, 0.8)', borderRadius: '20px', padding: '40px 30px', backgroundColor: 'rgba(255, 255, 255, 0.9)', flex: '1', minWidth: '300px', maxWidth: '400px', boxShadow: '0 10px 30px rgba(179, 139, 143, 0.25)' }}>
                ...
                <ul style={{ textAlign: 'left', listStyle: 'none', padding: 0, margin: '0 0 35px 0', color: '#736A6A', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.25rem', lineHeight: '2' }}>
                  <li>✧ Full access to Curation Quiz</li><li>✧ 5-piece premium box</li><li>✧ Monthly 1-on-1 consultations</li><li>✧ 25% off the boutique</li>
                </ul>
        ...
        {step === 3 && (
          <div style={{ animation: 'fadeIn 1s ease' }}>
            ...
            <p style={{ fontSize: '1.4rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', margin: '15px auto 35px' }}>Enter your details below to create your account.</p>
            {isLoading ? <EmpowermentLoader text={isSubscribing ? "Connecting to payment gateway..." : "Creating your account..."} /> : (
              <div style={{ maxWidth: '400px', margin: '0 auto' }}>
                ...
                <div style={{ marginTop: '35px' }}><EmpowermentButton text={isSubscribing ? "Proceed to Checkout" : "Create Account"} onClick={handleCreateAccount} /></div>
        ...
        {step === 4 && (
          <div style={{ animation: 'fadeIn 1s ease' }}>
            ...
            <p style={{ fontSize: '1.4rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', margin: '15px auto 35px' }}>Sign in to access your dashboard.</p>
            {isLoading ? <EmpowermentLoader text="Verifying credentials securely..." /> : (
              <div style={{ maxWidth: '400px', margin: '0 auto' }}>
                ...
                <div style={{ marginTop: '35px' }}><EmpowermentButton text="Sign In" onClick={handleLoginSubmit} /></div>
                <p style={{ cursor: 'pointer', color: '#A89999', textDecoration: 'underline', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem', marginTop: '20px' }} onClick={() => setStep(0)}>← Back</p>
              </div>
            )}
          </div>
          ...
          <div style={{ animation: 'fadeIn 1.5s ease' }}>
            ...
            {selectedTier === 'basic' ? (
               <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', border: '1px solid #E8C5C8', borderRadius: '15px', padding: '40px', boxShadow: '0 4px 15px rgba(232, 197, 200, 0.15)', margin: '40px 0' }}>
                 ...
                 <p style={{ fontSize: '1.2rem', color: '#736A6A', fontFamily: "'Cormorant Garamond', serif", marginBottom: '30px' }}>Your free membership grants you exclusive access to shop our beautiful collections. Upgrade anytime to receive a curated regimen blueprint.</p>
              ...
              <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', border: '1px solid #E8C5C8', borderRadius: '15px', padding: '40px', boxShadow: '0 4px 15px rgba(232, 197, 200, 0.15)', margin: '40px 0' }}>
                ...
                <p style={{ fontSize: '1.2rem', color: '#736A6A', fontFamily: "'Cormorant Garamond', serif", marginBottom: '30px' }}>Take the Consultation Quiz to generate your custom routine.</p>
              ...
              <>
                ...
                <div style={{ textAlign: 'left', marginBottom: '40px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
                    {getCuratedRecommendations().map((product) => <ProductCard key={product._id || product.title} product={product} onAddToCart={handleAddToCart} onCardClick={setSelectedProduct} />)}
                  </div>
                </div>
              </>
            )}
          ...
          <div style={{ animation: 'fadeIn 1.5s ease', textAlign: 'left', width: '100%' }}>
            ...
            {adminTab === 'orders' && (
               <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', border: '1px solid #E8C5C8', borderRadius: '15px', padding: '20px', overflowX: 'auto' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem' }}>
                    <thead><tr style={{ borderBottom: '2px solid #E8C5C8', textAlign: 'left', color: '#B38B8F' }}><th style={{ padding: '10px' }}>ID / Customer</th><th style={{ padding: '10px' }}>Total</th><th style={{ padding: '10px' }}>Status</th><th style={{ padding: '10px' }}>Actions</th></tr></thead>
                    <tbody>
                      {adminOrders.map((o) => (
            ...
            {adminTab === 'members' && (
               <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', border: '1px solid #E8C5C8', borderRadius: '15px', padding: '20px', overflowX: 'auto' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem' }}>
                    <thead><tr style={{ borderBottom: '2px solid #E8C5C8', textAlign: 'left', color: '#B38B8F' }}><th style={{ padding: '10px' }}>Name</th><th style={{ padding: '10px' }}>Tier</th><th style={{ padding: '10px' }}>Blueprint Details</th></tr></thead>
                    <tbody>
                      {adminMembers.map((m) => (
                          ...
                          <td style={{ padding: '10px', fontSize: '0.95rem' }}>
                            {m.blueprint ? (
                               Object.entries(m.blueprint).map(([key, val]) => (
                                 !['userId', '_id', 'createdAt', '__v'].includes(key) && val ? <span key={key} style={{ background: '#FFF0F2', padding: '2px 6px', borderRadius: '5px', margin: '2px', display: 'inline-block' }}>{key}: {val}</span> : null
                               ))
            ...
            {adminTab === 'products' && (
               <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', border: '1px solid #E8C5C8', borderRadius: '15px', padding: '20px', overflowX: 'auto' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'Cormorant Garamond', serif", fontSize: '1.1rem' }}>
                    <thead><tr style={{ borderBottom: '2px solid #E8C5C8', textAlign: 'left', color: '#B38B8F' }}><th style={{ padding: '10px' }}>Title</th><th style={{ padding: '10px' }}>Category Tags</th><th style={{ padding: '10px' }}>Action</th></tr></thead>
                    <tbody>
        ...
        {step >= 11 && step <= 20 && (
          <div style={{ animation: 'fadeIn 0.5s ease' }}>
            ...
            {step === 11 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Let's Begin</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>How would you describe your skin's natural temperament?</p><QuizOptionButton text="Balanced & Calm (Normal)" onClick={() => handleAnswer('skinType', 'Normal')} /><QuizOptionButton text="Thirsty & Delicate (Dry)" onClick={() => handleAnswer('skinType', 'Dry')} /><QuizOptionButton text="Naturally Dewy (Oily)" onClick={() => handleAnswer('skinType', 'Oily')} /><QuizOptionButton text="A Little Bit of Everything (Combination)" onClick={() => handleAnswer('skinType', 'Combination')} /></>)}
            {step === 12 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Your Vision</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>What is your primary focus for your skincare journey?</p><QuizOptionButton text="Age-Defying & Firming" onClick={() => handleAnswer('primaryGoal', 'Anti-Aging')} /><QuizOptionButton text="Deep Hydration & Plumping" onClick={() => handleAnswer('primaryGoal', 'Hydration')} /><QuizOptionButton text="Clearing & Balancing" onClick={() => handleAnswer('primaryGoal', 'Acne Control')} /><QuizOptionButton text="Ultimate Glow & Radiance" onClick={() => handleAnswer('primaryGoal', 'Radiance')} /></>)}
            {step === 13 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Your Environment</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>What best describes your daily climate or surroundings?</p><QuizOptionButton text="Humid & Tropical" onClick={() => handleAnswer('climate', 'Humid')} /><QuizOptionButton text="Arid, Dry, or Air-Conditioned" onClick={() => handleAnswer('climate', 'Dry')} /><QuizOptionButton text="Temperate & Balanced" onClick={() => handleAnswer('climate', 'Temperate')} /><QuizOptionButton text="Urban & Variable" onClick={() => handleAnswer('climate', 'Urban')} /></>)}
            {step === 14 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Delicate Details</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>How does your skin typically react to new active ingredients?</p><QuizOptionButton text="Resilient (Rarely reacts)" onClick={() => handleAnswer('skinSensitivity', 'Resilient')} /><QuizOptionButton text="Occasionally Sensitive" onClick={() => handleAnswer('skinSensitivity', 'Occasional')} /><QuizOptionButton text="Highly Reactive & Delicate" onClick={() => handleAnswer('skinSensitivity', 'Reactive')} /><QuizOptionButton text="Unpredictable" onClick={() => handleAnswer('skinSensitivity', 'Unpredictable')} /></>)}
            {step === 15 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Your Canvas</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>How would you describe your beautiful complexion depth?</p><QuizOptionButton text="Fair & Porcelain" onClick={() => handleAnswer('complexion', 'Fair')} /><QuizOptionButton text="Light & Peachy" onClick={() => handleAnswer('complexion', 'Light')} /><QuizOptionButton text="Medium & Golden" onClick={() => handleAnswer('complexion', 'Medium')} /><QuizOptionButton text="Tan, Olive, or Deep" onClick={() => handleAnswer('complexion', 'Deep')} /></>)}
            {step === 16 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Undertone Nuance</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>What is your skin's underlying undertone?</p><QuizOptionButton text="Cool (Pink/Blue hints)" onClick={() => handleAnswer('undertone', 'Cool')} /><QuizOptionButton text="Warm (Golden/Peach hints)" onClick={() => handleAnswer('undertone', 'Warm')} /><QuizOptionButton text="Neutral (Balanced mix)" onClick={() => handleAnswer('undertone', 'Neutral')} /><QuizOptionButton text="Olive (Rich golden-green)" onClick={() => handleAnswer('undertone', 'Olive')} /></>)}
            {step === 17 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Windows to the Soul</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>What is the captivating color of your eyes?</p><QuizOptionButton text="Warm Brown or Amber" onClick={() => handleAnswer('eyeColor', 'Brown')} /><QuizOptionButton text="Striking Blue or Grey" onClick={() => handleAnswer('eyeColor', 'Blue')} /><QuizOptionButton text="Enchanting Green" onClick={() => handleAnswer('eyeColor', 'Green')} /><QuizOptionButton text="Mesmerizing Hazel" onClick={() => handleAnswer('eyeColor', 'Hazel')} /></>)}
            {step === 18 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Your Silhouette</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>Which of these best describes your unique facial structure?</p><QuizOptionButton text="Softly Oval or Long" onClick={() => handleAnswer('faceShape', 'Oval')} /><QuizOptionButton text="Beautifully Round" onClick={() => handleAnswer('faceShape', 'Round')} /><QuizOptionButton text="Striking Square or Rectangle" onClick={() => handleAnswer('faceShape', 'Square')} /><QuizOptionButton text="Delicate Heart or Diamond" onClick={() => handleAnswer('faceShape', 'Heart')} /></>)}
            {step === 19 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Your Aesthetic</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>When it comes to makeup, what is your signature vibe?</p><QuizOptionButton text="Effortless & Barely There" onClick={() => handleAnswer('makeupVibe', 'Natural')} /><QuizOptionButton text="Soft, Romantic Glamour" onClick={() => handleAnswer('makeupVibe', 'Soft Glam')} /><QuizOptionButton text="Bold & Showstopping" onClick={() => handleAnswer('makeupVibe', 'Full Glam')} /><QuizOptionButton text="Creative & Trendsetting" onClick={() => handleAnswer('makeupVibe', 'Edgy')} /></>)}
            {step === 20 && (<><h2 style={{ fontSize: '2.5rem', color: '#B38B8F', fontFamily: "'Alex Brush', cursive", fontWeight: '400', marginBottom: '10px' }}>Regimen Focus</h2><p style={{ fontSize: '1.3rem', fontFamily: "'Cormorant Garamond', serif", color: '#736A6A', marginBottom: '30px' }}>What formulation texture do you prefer most in your daily ritual?</p><QuizOptionButton text="Lightweight Serums & Essences" onClick={() => handleAnswer('routineFocus', 'Serums')} /><QuizOptionButton text="Rich, Nourishing Creams" onClick={() => handleAnswer('routineFocus', 'Creams')} /><QuizOptionButton text="Long-Wear Pigments & Kits" onClick={() => handleAnswer('routineFocus', 'Kits')} /><QuizOptionButton text="Glossy, Dewy Finishes" onClick={() => handleAnswer('routineFocus', 'Gloss')} /></>)}
            ...
            {activeCascadingTags.length > 0 && (
              <div style={{ marginTop: '50px', paddingTop: '20px', borderTop: '1px solid rgba(232, 197, 200, 0.4)', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1rem', color: '#B38B8F', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 10px 0', fontWeight: 'bold' }}>Live Curation</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center', maxWidth: '600px', marginBottom: '15px' }}>
                  {activeCascadingTags.map((tag, i) => (
                    <span key={i} style={{ backgroundColor: 'rgba(232, 197, 200, 0.2)', color: '#736A6A', padding: '4px 12px', borderRadius: '15px', fontSize: '0.9rem', border: '1px solid #E8C5C8', fontFamily: "'Cormorant Garamond', serif" }}>+ {tag}</span>
                  ))}
                </div>
              </div>
            )}
